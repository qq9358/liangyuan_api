...
namespace Egoal.Events.Bus.Entities
{
    public class EntityChangeEventHelper : IEntityChangeEventHelper
    {
            ...
            foreach (var changedEntity in changedEntities)
            {
                switch (changedEntity.ChangeType)
                {
                    ...
                    case EntityChangeType.Deleted:
                        ...
                        break;
                    default:
                        throw new TmsException("Unknown EntityChangeType: " + changedEntity.ChangeType);
                }
            }
        }
        ...
        protected virtual void TriggerDomainEvents(List<DomainEventEntry> domainEvents)
        {
            ...
            {
                _eventBus.Trigger(domainEvent.EventData.GetType(), domainEvent.SourceEntity, domainEvent.EventData);
            }
        }
        ...
        protected virtual void TriggerEventWithEntity(Type genericEventType, object entity, bool triggerInCurrentUnitOfWork)
        {
            ...
            {
                _eventBus.Trigger(eventType, (IEventData)Activator.CreateInstance(eventType, new[] { entity }));
                return;
            }

            _unitOfWorkManager.Current.Completed += (sender, args) => _eventBus.Trigger(eventType, (IEventData)Activator.CreateInstance(eventType, new[] { entity }));
        }
    }
}
