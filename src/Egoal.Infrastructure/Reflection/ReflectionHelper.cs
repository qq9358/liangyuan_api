...
namespace Egoal.Reflection
{
    public static class ReflectionHelper
    {
        ...
        public static string GetFieldNameByValue<TType, TValue>(TValue value)
        {
            var fields = typeof(TType).GetFields();
            foreach (var field in fields)
            {
                if (value.Equals(Convert.ChangeType(field.GetValue(null), typeof(TValue))))
                {
                    return field.Name;
                }
            }
